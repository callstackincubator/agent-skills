...
It clones the remote repository, copies its `plugins/` payload into the matching `.codex/plugins/<repo>/plugins` location, and merges marketplace entries from `plugins/manifest.json` into the correct `marketplace.json` file for the selected scope.
