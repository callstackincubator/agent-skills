...
describe("discoverPackageJsonPaths", () => {
  it("finds package.json files across a monorepo", async () => {
    ...
    expect(packageJsonPaths.some((entry) => entry.includes("node_modules"))).toBeFalse();
  });
});
...
describe("buildSkillPlan", () => {
  it("detects missing and extra RN skills", () => {
    const plan = buildSkillPlan(
      {
        packageJsonPaths: ["/repo/package.json"],
        libraries: ["@testing-library/react-native", "react-native", "react-native-reanimated"]
      },
      [
        {
          name: "github",
    ...
    expect(plan.ignoredInstalledSkills.map((skill) => skill.name)).toEqual(["github"]);
  });
  ...
  it("marks managed but unmatched installed skills as extra", () => {
    ...
    expect(plan.missingSkills.map((skill) => skill.name)).toEqual(["react-native-testing"]);
    expect(plan.extraInstalledSkills.map((skill) => skill.name)).toEqual(["react-native-brownfield-migration"]);
    expect(plan.ignoredInstalledSkills.map((skill) => skill.name)).toEqual(["github"]);
  });
});
